...
func main() {
	...
	if err := run(os.Args[1:]); err != nil {
		log.Fatalf("run JSONL copy workload: %v", err)
	}
}
...
func run(arguments []string) error {
	...
	if len(invocation.ExecutableArgs) != 0 {
		return errors.New("the JSONL copy workload accepts no executable arguments")
	}
	...
	temporary, err := os.CreateTemp(filepath.Dir(outputFilename), ".mill-jsonl-copy-*.tmp")
	if err != nil {
	...
	written, err := io.Copy(temporary, io.NewSectionReader(input, invocation.InputStartByte, length))
	if err != nil {
		return fmt.Errorf("copy input range: %w", err)
	}
	if written != length {
		return fmt.Errorf("copied %d bytes, want %d", written, length)
	}
